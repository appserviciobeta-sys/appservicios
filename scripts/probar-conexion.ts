...
/// Huella de la contraseña: los primeros 10 hex de un SHA-256. Sirve para
/// comparar con la de producción (en /api/salud) sin que la contraseña
/// aparezca nunca en pantalla ni en un chat.
function huella(clave: string): string {
  return createHash("sha256").update(clave).digest("hex").slice(0, 10);
}
...
async function revisar(r: Revision) {
  ...
  console.log(`   contraseña  ${p.largoClave} caracteres · huella ${p.huellaClave}`);
  ...
  try {
    ...
    if (/password authentication failed|SASL|SCRAM/i.test(mensaje)) {
      console.log("     La contraseña no coincide con la de Supabase.");
      console.log("     Compara la huella de arriba con la de producción:");
      console.log("     https://appservicios-ten.vercel.app/api/salud  →  claveHuella");
      console.log("     Si son distintas, la de este archivo .env es la equivocada.");
